...
exports.register = async (req, res) => {
  try {
    ...
    const newUser = manager
      ? new User({ email, password, name, manager })
...
exports.verifyJWT = async (req, res) => {
  jwt.verify(req.body.auth, process.env.JWT_SECRET, (err, decoded) => {
    if (err) {
      res.status(401);
    } else {
...
exports.getEmployees = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ employees, numEmployees });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
